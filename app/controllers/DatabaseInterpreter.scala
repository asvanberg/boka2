...
object DatabaseInterpreter extends (Execution ~> ({type λ[α] = Reader[Connection, α]})#λ) {
  override def apply[A](fa: Execution[A]): Reader[Connection, A] = Reader {
    implicit connection ⇒
      fa match {
        case Pure(a) ⇒ a
        case Execute(command) ⇒ command match {
