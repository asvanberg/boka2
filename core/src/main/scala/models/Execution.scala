...
final case class Execute[A] private[models] (command: InventoryManagement[A]) extends Execution[A]

private[models] object Execution {
 def pure[A](a: A): FreeC[Execution, A] =
   Free.liftFC(Pure(a))

 def execute[A](command: InventoryManagement[A]): FreeC[Execution, A] =
