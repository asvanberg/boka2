...
class Copy
  ...
  @add: (product, copyData) ->
    m.secureRequest(
      method: "POST"
      url: "/api/product/#{product.id()}/copy"
      data: copyData
      type: Copy
    )
...
form =
  ...
  view: (ctrl, args) ->
    ...
    m "form", {onsubmit: (e) -> e.preventDefault(); args.onsave(product).then(null, ctrl.error)}, [
      if ctrl.error() then m ".alert.alert-danger", [
        m "ul", [m "li", message for _, message of ctrl.error()]
      ]
      m ".form-group", {class: if ctrl.error()?.name then "has-error" else ""}, [
      ...
      m ".form-group", {class: if ctrl.error()?.description then "has-error" else ""}, [
